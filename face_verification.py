...
#             with os.scandir("database/pranav") as it:
#                 for entry in it:
...
def verifyFace():
    ...
    while(True):
        ...
        image = capture.read()
        try:
            print("Comparing for frame " + str(frames))
            # with os.scandir("database/pranav") as it:
            #     for entry in it:
            #         if entry.name.endswith(".jpg") and entry.is_file():
            #             result = DeepFace.verify(entry.path, image, detector_backend = 'opencv', model_name = "ArcFace")
            ...
            df = DeepFace.find(image,'database/pranav',model_name='ArcFace',detector_backend='opencv')
            if len(df):
                count += 1

            if count==5:
                print("FACE VERIFIED FIVE TIMES SUCCESSFULLY!")
                capture.release()
                cv2.destroyAllWindows()
                return True
            frames += 1
